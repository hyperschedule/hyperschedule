...
function timeStringForAdjust(timeString) {
  let [hours, minutes] = timeStringToHoursAndMinutes(timeString);
  hours %= 24;
  return (
    hours.toString().padStart(2, "0") +
...
function createSlotEntities(course, slot) {
  ...
  for (const slot of course.courseSchedule) {
    ...
    const timeSince8am = startTime - timeStringToHours("08:00");
    ...
    for (const slot of course.courseSchedule) {
      ...
      for (const day of slot.scheduleDays) {
        for (const [left, right] of getConsecutiveRanges(slot.scheduleTerms)) {
          ...
          const yStart =
            (startHours - timeStringToHours("08:00") + startMinutes / 60) *
              rowHeight +
            0.75 * 72;

          const yEnd =
            (endHours - timeStringToHours("08:00") + endMinutes / 60) *
              rowHeight +
            0.75 * 72;
