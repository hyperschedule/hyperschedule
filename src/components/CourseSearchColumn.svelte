<script lang="ts">
  ...
  import {apiData, showClosedCourses, hideAllConflictingCourses, hideStarredConflictingCourses} from "../lib/app/local";
...
function processSearchText(q: string) {
  ...
  for (let text of searchText) {
    ...
    if (
      filterKeywordsValues.some((filter: string) => 
...
function courseConflictWithSchedule(
  ...
  $: filteredCourseKeys = $apiData === null ? [] :
